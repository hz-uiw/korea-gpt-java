...
/*
    ...
    인스턴스 생성:
        - 생성자가 인스턴스 생성하는 것이 아닌, new 연산자 키워드가 인스턴스를 생성한다
 */

class Example1 {
    int value;

    // 컴파일러가 제공하는 기본 생성자(매개변수가 없고 아무런 구현이 안되어있음)가 생략
}
class Example2 {
    int value;

    // 사용자 정의 생성자: 컴파일러가 자동으로 기본 생성자를 제고
    Example2(int value) {
        this.value = value;
    }

    // 빈 생성자를 직접 명시해야만 사용 가능: 사용자 정의 생성자가 1개라도 있는 경우
//    Example2() {}
}
...
class Car {
    ...
    Car (String color) {
        // 정의 생성자에서 다른 생성자 호출 시: this() 사용:
            // 생성자 이름으로 클래스 이름 대신 this를 사용
            // !! 정의 생성자에서 다른 생성자 호출 시 반드시 첫 줄에만 호출 가능
