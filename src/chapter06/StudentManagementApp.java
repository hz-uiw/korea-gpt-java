...
public class StudentManagementApp {
    ...
    // - 프로그램 시작시 JVM이 접근해야하기 때문에 반드시 public
    
    // - static: 정적 메서드, 메서드가 인스턴스 생성 없이 호출됨
        // JVM이 클래스의 객체를 생성하지 않고도 메서드를 직접 호출하기 위해 필요함
    
    // 'main': JVM이 프로그램 실행시 가장 먼저 찾는 메서드 이름 - 반드시 main이어야 함
    ...
    //  main 메서드 역할:
        ...
        // - 클래스에 메인 메서드가 없을 경우 JVM은 실행시 오류를 발생

    static ArrayList<Student> studentList = new ArrayList<>();

    public static void main(String[] args) {
        // 학생 추가
        addStudent(1, "권민창", 25, "GPT반", 4.0);
        addStudent(2, "권영진", 22, "IoT반", 3.8);
        addStudent(3, "정현영", 28, "GPT반", 4.3);
        addStudent(4, "윤동현", 27, "빅데이터반", 4.1);

        printAllStudents();

        findStudentByID(3); // ID: 3, Name: 정현영, Age: 28, Major: GPT반, GPA: 4.3
