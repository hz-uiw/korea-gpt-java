...
/*
    ...
    자식 클래스 객체를 생성할 때, 자식 클래스는 해당 클래스의 생성자를 통해 객체 생성:
        - 자식 클래스의 생성자 호출시 '항상' 부모 클래스의 생성자가 호출!
        - 사용자 정의 생성하 호출이 필요하지는 않다
 */
/*
    Mammal 클래스
...
class Mammal {
    ...
    Mammal(String name) {
        // 필드와 지역변수의 이름이 같은 경우 충돌 방지를 위해 필드에 this. 키워드로 구분
...
//    }
    ...
    Cat() {
        // super();
        // : 자식 클래스의 기본 생성자는 사용자 정의 생성자가 호출이 없더라도 super() 부모 클래스 생성자 호출 키워드가 정의되어 있음
//        super("여기는 자식 클래스의 생성자입니다.");
    ...
    Cat(String name) {
        // super()는 부모 클래스의 생성자를 가져오는 키워드: 부모 클래스 내에 정의된 생성자 형태 그대로를 사용
        super(name);
