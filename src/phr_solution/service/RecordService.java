...
public class RecordService implements RecordServiceInterface {
    ...
    // 환자 ID 유효성 검증
    private void validDatePatientId(Long patientId) {
        // 환자 존재 여부 확인
        Optional<Patient> foundPatient = patientRepository.findById(patientId);

        if (!foundPatient.isPresent()) {
            throw new IllegalArgumentException("해당 ID의 환자를 찾을 수 없습니다.\nID: " + patientId);
        }
    }
    ...
    public void createRecord(RecordRequestDto requestDto) {
        try {
            validDatePatientId(requestDto.getPatientId());
    ...
    public void deleteRecord(Long id) {
        try {
            HealthRecord healthRecord = recordRepository.findById(id)
                    .orElseThrow(() -> new IllegalArgumentException("해당 ID의 건강 기록은 존재하지 않습니다.\nID:" + id));
