...
public class App {
    ...
    public static void main(String[] args) {
        try {
            while(true) {
                ...
                int choice = getChoice(); // 사용자의 메뉴 선택 저장
                if (!processChoice(choice)) break; // 사용자의 선택을 처리 (처리의 반환값이 false면 while문 종료
            }
        } finally {
            scanner.close();
        }
    ...
    private static boolean processChoice(int choice) {
        switch (choice) {
            ...
            case 2: { // 환자 정보 조회 (전체)
                ...
                if(patients.isEmpty()) System.out.println("환자 정보가 없습니다.");
            ...
            case 3: { // 환자 정보 조회 (단건): 환자 고유 번호
                ...
                if (patient == null) System.out.println("해당하는 ID의 환자가 없습니다.");
            ...
            case 7: { // 건강 기록 조회(전체)
                ...
                if (records.isEmpty()) System.out.println("건강 기록이 없습니다.");
                else records.forEach(System.out::println);
                break;
            }
            case 8: { // 건강 기록 조회(조건부 검색 - 진단명으로 조회): 진단명
            ...
            default: {
                System.out.println("잘못된 선택입니다. 유효한 매뉴를 선택해주세요");
                break;
            }
        }
        return true;
    ...
    private static long getIdInput() {
        ...
        String input = getInput("아이디를 입력해주세요 >>> ");
        return Long.parseLong(input);
    }

    private static String getInput (String prompt) {
        System.out.print(prompt);
        // trim() 메서드 문자열 앞뒤의 공백을 제거
