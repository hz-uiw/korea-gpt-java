...
// - 주로 DB에서 데이터를 가져오거나 REST API 통신에서 요청 및 응답 데이터를 담아 전달할 때 사용
...
// 2) VO(Value Object)
// 값을 표현하는 객체
// - 데이터를 담는 용도로 사용, 불변성을 가짐(한 번 생성되면 내용 변경이 불가!)
// - 주로 필드값을 final 설정
// - Getter만을 가짐
@AllArgsConstructor
@Getter
...
class UserDao {
    // 로그인 한 사용자 데이터 단건 조회
    public UserSignInResponseDto getUserById(String name) {
