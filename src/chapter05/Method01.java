...
/*
    ...
    메서드의 구조:
        ReturnType methodName(Parameter List) {
            ...
            1. ReturnType(반환 유형):
                - 메서드가 작업을 수행한 후 반환하는데 데이터의 유형을 지정
                - 반환 유형이 없는 경우 void로 지정

            2. methodName(메서드 이름):
                - 메서드를 식별하는 이름
                - 명명 규칙: lowerCamelCase
                - (권장 사항); 동사를 사용하여 메서드의 작업을 설명
                ex) '먹다'는 eat이라는 동사로 사용, eating이라는 명사로 사용하지 않는다

            3. Parameter List(매개변수 목록):
                - 메서드에 전달되는 입력 값들의 목록
                - 0개 이상으로 지정 => 지정하지 않을 수 있으며 원하는 개수만큼 전달도 가능
                - 각 매개변수는 자료형과 함께 선언되어야 함;
                    - 일반 변수 정의처럼 동일한 데이터 타입의 생략이 불가
                    - 각 메서드 내부에서 지역 변수로 사용
...
class Calculator {
    ...
    void voidMethod() {
        // 메서드의 void 반환 타입:
            // return 키워드가 없거나 키워드 뒤의 값이 없는 경우 반드시 지정하여야 함
...
public class Method01 {
    public static void main(String[] args) {
        // 메서드 사용:
            // - 해당 메서드가 정의된 클래스의 객체 생성
            // - 생성된 객체를 사용하여 메서드 호출(. 연산자 사용)
