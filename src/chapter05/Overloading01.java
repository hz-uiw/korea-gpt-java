...
/*
    ...
    (메서드) 오버로딩:
        - overloading(중복 정의): 한 클래스 내에서 같은 이름의 '메서드'를 여러 개 정의 하는 것

        1. 오버로딩의 성립 조건:
            - 메서드 이름은 동일해야 함
            - 매개변수의 개수 또는 타입이 달라야 함 => 이름만 같아야함
            - 반환타입은 오버로딩 성립에 영향이 없음

        2. 오버로딩의 장점:
            - 사용자가 메서드의 매개변수 순서를 외우지 않아도 사용 가능
            - 작업의 본질이 동일한 메서드를 하나의 이름으로 그룹화 가능 => 코드의 재사용성, 가독성 향상
            - 컴파일 시 타입 체크 강화: 올바른 매개변수를 가진 메서드를 자동 선택
...
class Calc {
    ...
    // long add(int c, int d) {return c + d;} - Error => 실제 값 차이 여부와 반환타입의 차이 여부는 오버로딩과 관련 없음
