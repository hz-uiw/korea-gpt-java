    ...
    메서드의 구조:
        ...
        1. 메서드 선언부:
            ...
            2) 매개변수(parameter) 선언:
                ...
                - 변수들의 데이터 타입이 같더라도 타입 생략은 안된다

        2. 메서드의 구현부:
            - 메서드 선언부 다음에 오는 중괄호 {}
            - 반환 타입이 void가 아닐 경우 return문을 반드시 포함
            - 해당 반환값은 반환 타입과 일치하거나 자동 형 변환이 가능한 데이터이어야만 함

            ex) double addDouble(int a, int b) {
                int sum = a + b;
                return sum;
            }
...
public class Method02 {
    public static void main(String[] args) {
        ...
        // 함수의 파라미터에 데이터 전달:
            // 함수 호출시 소괄호() 영역 내에서 parameter에 전달되는 값을 argument라고 부름
        
        // 매개변수(Parameter)와 인자(인수, Argument)
        
        // 1. 매개변수:
            // - 메서드가 정의할 때 괄호 안에 선언되는 '변수'
            // - 메서드가 호출될 때 값을 전달받기 위한 입력변수로 사용
            // > 메서드의 선언부에 정의, 호출시 전달받은 값은 내부적으로 사용
        
        // 2. 인자(인수):
            // - 메서드를 호출할 때 괄호 안에 전달하는 '값'
            // - 호출된 메서드의 매개변수에 실제 값을 전달하는 역할을 함
        ...
        System.out.println(example.add(10, 20));    // 10과 20은 인자
    }
}
