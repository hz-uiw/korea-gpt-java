...
// 자바에서 배열과 리스트의 정렬
    ...
    // 2. 리스트(List 클래스) 정렬
    // : Collections.sort() 도는 List 클래스의 sort() 메서드를 사용
import java.util.*;
...
public class A_Sort {
    public static void main(String[] args) {
        // 1. 배열 정렬
        // 1) 오름차순 정렬
        int[] numbers1 = {5, 3, 2, 8, 1};
        Arrays.sort(numbers1);
        System.out.println(Arrays.toString(numbers1)); // [1, 2, 3, 5, 8]

        // 2) 내림차순 정렬 - Comparator 클래스 내의 reverseOrder() 정적 메서드 사용
        // Collections.reverseOrder()는 comparator<T>를 반환하는 메서드
        // : 객체간의 비교를 실시
        // - 기본데이터 타입에 대한 비교를 하지 않고, Comparator는 객체 타입에 대한 비교를 한다
        ...
        List<String> names1 = new ArrayList<>(List.of("Tom", "Jerry", "Pororo", "Ruffy"));
        // 문자열 길이 기준 정렬
...
//        names.sort(Comparator.comparingInt(name -> name.length()));
        System.out.println(names1); // [Tom, Jerry, Ruffy, Pororo]
        ...
        List<String> characterNames = new ArrayList<>(List.of("Pororo", "Jerry", "Ruffy", "Tom"));
        ...
        fruits.sort(Comparator.comparingInt(String::length)
                ...
                // : 두개의 문자열 값을 비교하여 int값으로 반환해주는 함수
