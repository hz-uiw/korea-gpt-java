        ...
        /*
            배열과 리스트
                ...
                - List(리스트):
                    ...
                    - List의 Index 번호는 데이터의 저장 위치만 확인하는 의미
                    - 불연속적 메모리 공간을 차지
                    - Array와 달리 동적이며 배열의 크기가 정해져있지 않음
         */
        /*
            +) List<> 타입 변수에는 List.of()와 ArrayList를 통해 생성된 불변 리스트, 가변 리스트 모두 할당 가능
            List.of() 메서드:
                - 읽기 전용(불변) 리스트를 생성하는데 사용
                - 수정(추가, 수정, 변경) 불가, null 값을 포함할 수 없음
         */
...
//         asList.add("수박");
        ...
        | 항목              | new ArrayList<>()       | Arrays.asList()      | List.of()            |
        |-------------------|-------------------------|----------------------|----------------------|
        | 1. 구현 방식      | 생성자 방식             | Arrays 클래스의 asList 메서드 | List 인터페이스의 of 메서드 |
        | 2. 원소 추가, 삭제 | 됨                      | 안됨                 | 안됨                 |
        | 3. 원소 수정      | 됨                      | 됨                   | 안됨                 |


            +) Arrays.asList()가 반환하는 리스트는 가짜 ArrayList
            Arrays 클래스 내부에 구현된 별개의 ArrayList (컬렉션 프레임워크가 아님)
        */
    }
}
