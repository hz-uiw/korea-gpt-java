        ...
        종류:
            ...
            3. 메서드 내부 클래스(지역 클래스):
                "메서드 내에 정의된 내부 클래스"
                    >> 해당 메서드 내에서만 사용 가능
                    >> 메서드의 지역 변수에만 접근 가능
                    cf) 로컬 변수(외부 클래스 멤버 변수)가 final인경우에만 접근 가능

            4. 익명 (내부) 클래스:
                이름이 없는 내부 클래스 -> 주로 인터페이스나 추상 클래스의 구현에 사용
                    >> 즉시 객체 생성 가능
...
public class A_Inner {
    public static void main(String[] args) {
        ...
        // 1) 외부 클래스 인스턴스화 필요 없음, 클래스를 통한 내부 클래스 생성이 가능
        ...
        AbstractClass abstractClass2 = new AbstractClass() {
            @Override
            void display() {
                System.out.println("두 번째 일명 내부 클래스");
            }
        };
